...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, device, return_all=False, attn_self=False, return_attn_map = False, layer='final'):
        x = x.to(device)
        _, _, w, h = x.shape
        if return_attn_map:
            return self.get_attn(x, layer=layer, attn_self=attn_self)
        ...
        for blk in self.transformer.resblocks[:-1]:
            ...
            if layer_count in selected_intermediate_layers:
                ...
                attn_list.append(saved_attn) # attention maps from intermediate layers
                x = blk(x)
            else:
                x = blk(x)
    ...
    def use_saved_attn(self, attn_layer, x, saved_attn):
        ...
        attn_output = torch.bmm(saved_attn, v)
    ...
    def get_attn(self, x, layer='all', attn_self=False):
        ...
        if layer == 'final':
            ...
            attn_map = self.custom_attn(self.transformer.resblocks[-1].attn,
                                        ...
                                        attn_self=attn_self, return_attn=True)
            return attn_map
        elif layer == 'all':
            attn_map = []
            for blk in self.transformer.resblocks[:-1]:
            ...
            for blk in self.transformer.resblocks[-1:]:
                x_i, attn_i = self.custom_attn(blk.attn, blk.ln_1(x), with_attn=True, attn_self=attn_self)
                x = x + x_i
                x = x + blk.mlp(blk.ln_2(x))
                attn_map.append(attn_i)
            return attn_map
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_all=False, attn_self=False, return_attn_map = False, layer='final', device='cuda:0'):
        return self.visual(image.type(self.dtype), return_all=return_all, attn_self=attn_self, return_attn_map=return_attn_map, layer=layer, device=device)
